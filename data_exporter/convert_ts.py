...
def convert_yolo_to_torchscript(model_path, output_path):
    ...
    if isinstance(model, dict):
        if "model" in model:
            ...
            print(
                "ERROR: This appears to be a state dict. You need the model architecture to convert it."
            )
            return False
        else:
    ...
    try:
        ...
        with torch.no_grad():
            ...
            print(
                f"✅ Verification passed. Output shape: {test_output.shape if hasattr(test_output, 'shape') else type(test_output)}"
            )
        ...
        try:
            ...
            try:
                ...
                traced_wrapper = torch.jit.trace(
                    wrapper, dummy_input, strict=False
                )
...
convert_yolo_to_torchscript(
    "/nfs/exports/ratlv/detection/yolo/yoloball/best.pt",
    "/nfs/exports/ratlv/detection/yolo/yoloball/best_ts.pt",
)
