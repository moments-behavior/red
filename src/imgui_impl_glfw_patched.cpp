...
#include <CoreGraphics/CoreGraphics.h>  // for CGEventSourceFlagsState (modifier key fix)
#endif
#ifndef _WIN32
...
#ifdef __APPLE__
    // On macOS, use CoreGraphics to query actual hardware modifier state.
    // glfwGetKey() caches key state from events, but macOS screen recording
    // (Cmd+Shift+5) can swallow key-up events, leaving GLFW with permanently
    // stuck modifier keys. CGEventSourceFlagsState reads the real hardware state.
    // Map physical keys to ImGui modifiers (same mapping as original GLFW code):
    //   Physical Ctrl    → ImGuiMod_Ctrl
    //   Physical Cmd     → ImGuiMod_Super  (ImGui swaps Ctrl/Super on macOS internally)
    //   Physical Shift   → ImGuiMod_Shift
    //   Physical Option  → ImGuiMod_Alt
