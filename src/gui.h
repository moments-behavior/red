...
static void gui_plot_keypoints(KeyPoints *keypoints, SkeletonContext *skeleton, int view_idx, u32 animal_idx, int num_cams, bool is_active)
{
    float pt_size = 6.0f;
    for (u32 node=0; node < skeleton->num_nodes; node++){
        if (keypoints->keypoints2d[view_idx][node].is_labeled){
            ...
            bool drag_point_clicked, drag_point_hovered, drag_point_modified;
            ...
            if (drag_point_hovered) {
                ...
                if (ImGui::IsKeyPressed(ImGuiKey_F, false)) // Delete hovered keypoints from all the views
                {
                    for (int cam_idx =0; cam_idx < num_cams; cam_idx++) {
                        ...
                        keypoints->active_kp_id[cam_idx] = node;
                    }
                }
            }
