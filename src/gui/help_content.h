    ...
    static const std::vector<Group> g = {
        ...
        {"Keypoints window", "Selecting, copying & deleting keypoint columns (keys act while the Keypoints window is hovered or focused)", Gate::Always, {
            ...
            {S::PasteKeypoints, nullptr, "Paste the copied keypoints onto this frame", Gate::Always,
                 "Overwrites those keypoints on the target frame and clears the column selection"},
            {S::DeleteKeypoint, nullptr, "Delete the hovered keypoint", Gate::Always,
                 "Over a cell: that camera. Over a name: all cameras. Also deletes a single selected column"},
            {S::DeleteSelectedKp, nullptr, "Delete the selected keypoint set from all cameras", Gate::Always,
                 "Required when 2+ columns are selected (plain Delete only shows a reminder)"},
        }},
