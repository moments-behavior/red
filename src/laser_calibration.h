...
// Visualization: run detection pipeline and produce RGBA mask image.
// Output rgba_out must be pre-allocated to width*height*4 bytes.
// Returns number of valid blobs found (0, 1, or more).
// If exactly 1 valid blob, fills det with its centroid.
struct DetectionVizResult {
    int num_blobs = 0;          // valid blobs after area filtering
    int total_mask_pixels = 0;  // pixels passing threshold
    SpotDetection det;          // valid only if num_blobs == 1
};

// Pre-allocated work buffers for detection viz — eliminates ~100MB alloc/free
// per call. Create one per camera, reuse across frames.
struct DetectionVizBuffers {
    std::vector<uint8_t> mask;
    std::vector<uint8_t> eroded;
    std::vector<uint8_t> dilated;
    std::vector<int16_t> labels;  // blob label per pixel (0 = none)
    int last_npixels = 0;

    void ensure(int npixels) {
        if (npixels != last_npixels) {
            mask.resize(npixels);
            eroded.resize(npixels);
            dilated.resize(npixels);
            labels.resize(npixels);
            last_npixels = npixels;
        }
    }
};

inline DetectionVizResult detect_laser_spot_viz(
    const uint8_t *pixels, int width, int height, int stride,
    LaserPixelFormat fmt, int green_threshold, int green_dominance,
    int min_blob_pixels, int max_blob_pixels, uint8_t *rgba_out,
    DetectionVizBuffers *bufs = nullptr) {

    DetectionVizResult result;
    int npixels = width * height;
    int bpp = (fmt == LASER_FMT_BGRA) ? 4 : 3;
    int r_off = (fmt == LASER_FMT_BGRA) ? 2 : 0;
    int b_off = (fmt == LASER_FMT_BGRA) ? 0 : 2;

    // Use pre-allocated buffers if provided, else local
    DetectionVizBuffers local_bufs;
    DetectionVizBuffers &b = bufs ? *bufs : local_bufs;
    b.ensure(npixels);

    uint8_t *mask = b.mask.data();
    uint8_t *eroded = b.eroded.data();
    uint8_t *dilated = b.dilated.data();
    int16_t *labels = b.labels.data();

    // Step 1: Threshold — binary mask + RGBA output in one pass
    // Also write the RGBA output here to avoid a separate pass later
    memset(rgba_out, 0, npixels * 4);  // black background with alpha=0
    int mask_count = 0;
    for (int y = 0; y < height; y++) {
        const uint8_t *row = pixels + y * stride;
        int row_off = y * width;
        for (int x = 0; x < width; x++) {
            uint8_t r = row[x * bpp + r_off];
            uint8_t g = row[x * bpp + 1];
            uint8_t bv = row[x * bpp + b_off];
            if (g > green_threshold && g >= r && g >= bv &&
                (  (g > r + green_dominance && g > bv + green_dominance)
                || (g > 200 && g >= r && g >= bv)  )) {
                mask[row_off + x] = 1;
                mask_count++;
                // Tentatively gray — will be upgraded if it survives morphology
                uint8_t *out = rgba_out + (row_off + x) * 4;
                out[0] = 60; out[1] = 60; out[2] = 60; out[3] = 255;
            } else {
                mask[row_off + x] = 0;
                // Set alpha for black pixels
                rgba_out[(row_off + x) * 4 + 3] = 255;
            }
        }
    }
    result.total_mask_pixels = mask_count;

    // Early out: if very few mask pixels, skip morphology
    if (mask_count < min_blob_pixels) {
        return result;
    }

    // Step 2: Erode (3x3) — only iterate over rows with potential mask pixels
    memset(eroded, 0, npixels);
    for (int y = 1; y < height - 1; y++) {
        int row_off = y * width;
        for (int x = 1; x < width - 1; x++) {
            int idx = row_off + x;
            if (!mask[idx]) continue;
            // Check all 9 neighbors (3x3 kernel)
            if (mask[idx - width - 1] & mask[idx - width] & mask[idx - width + 1] &
    ...
    // Step 3: Dilate (3x3)
    memset(dilated, 0, npixels);
    for (int y = 1; y < height - 1; y++) {
        int row_off = y * width;
        for (int x = 1; x < width - 1; x++) {
            int idx = row_off + x;
            if (eroded[idx - width - 1] | eroded[idx - width] | eroded[idx - width + 1] |
                eroded[idx - 1]         | eroded[idx]          | eroded[idx + 1] |
                eroded[idx + width - 1] | eroded[idx + width] | eroded[idx + width + 1])
                dilated[idx] = 1;
        }
    }

    // Step 4: Floodfill connected components on the dilated mask.
    // Only visits non-zero pixels — typically a few hundred, so very fast.
    memset(labels, 0, npixels * sizeof(int16_t));
    struct BlobInfo { int size; double sum_x, sum_y, sum_w; };
    std::vector<BlobInfo> blobs;  // small — typically 0-3 blobs
    ...
    for (int y = 1; y < height - 1; y++) {
        ...
        for (int x = 1; x < width - 1; x++) {
            ...
            // BFS floodfill from this pixel
            int16_t lbl = next_label++;
            BlobInfo blob{0, 0.0, 0.0, 0.0};
            // Use a small stack-based queue (blob pixels are few)
            ...
            for (size_t qi = 0; qi < queue.size(); qi++) {
                int ci = queue[qi];
                int cx = ci % width, cy = ci / width;
                blob.size++;
                double gw = pixels[cy * stride + cx * bpp + 1]; // green weight
                blob.sum_x += cx * gw;
                blob.sum_y += cy * gw;
                blob.sum_w += gw;
                ...
                for (int ni : neighbors) {
                    if (ni >= 0 && ni < npixels && dilated[ni] && !labels[ni]) {
                        labels[ni] = lbl;
                        queue.push_back(ni);
                    }
                }
    ...
    // Step 5: Classify blobs and color the RGBA output
    int valid_blob_idx = -1;
    for (int bi = 0; bi < (int)blobs.size(); bi++) {
        if (blobs[bi].size >= min_blob_pixels && blobs[bi].size <= max_blob_pixels) {
            result.num_blobs++;
            valid_blob_idx = bi;
        }
    }

    // Color dilated pixels based on their blob classification
    for (int y = 1; y < height - 1; y++) {
        int row_off = y * width;
        for (int x = 1; x < width - 1; x++) {
            int idx = row_off + x;
            if (!dilated[idx]) continue;
            int bi = labels[idx] - 1;  // blob index (0-based)
            uint8_t *out = rgba_out + idx * 4;
            if (bi >= 0 && bi < (int)blobs.size()) {
                int sz = blobs[bi].size;
                if (sz >= min_blob_pixels && sz <= max_blob_pixels) {
                    out[0] = 0; out[1] = 255; out[2] = 0; out[3] = 255; // green
                } else if (sz < min_blob_pixels) {
                    out[0] = 200; out[1] = 200; out[2] = 0; out[3] = 255; // yellow
                } else {
                    out[0] = 255; out[1] = 50; out[2] = 50; out[3] = 255; // red
                }
            }
        }
    }

    // Centroid for single valid blob
    if (result.num_blobs == 1 && valid_blob_idx >= 0) {
        auto &vb = blobs[valid_blob_idx];
        if (vb.sum_w > 1e-9) {
            result.det.cx = vb.sum_x / vb.sum_w;
            result.det.cy = vb.sum_y / vb.sum_w;
            result.det.pixel_count = vb.size;
        }
    }
