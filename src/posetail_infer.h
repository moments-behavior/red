...
inline void crop_and_resize_rgba_to_rgb01(const uint8_t *rgba, int src_w,
    ...
    for (int y = 0; y < out_h; ++y) {
        ...
        for (int x = 0; x < out_w; ++x) {
            ...
            for (int c = 0; c < 3; ++c) {
                // Output is RGB; the source may be BGRA (see POSETAIL_SRC_*).
                const int sc = (c == 0) ? POSETAIL_SRC_R
                             : (c == 2) ? POSETAIL_SRC_B : 1;
                float v00 = rgba[(y0 * src_w + x0) * 4 + sc];
                float v01 = rgba[(y0 * src_w + x1) * 4 + sc];
                float v10 = rgba[(y1 * src_w + x0) * 4 + sc];
                float v11 = rgba[(y1 * src_w + x1) * 4 + sc];
