...
//   src/stb_image_write.h for the PNG encode (the implementation is compiled
//   once in red.cpp via STB_IMAGE_WRITE_IMPLEMENTATION). This branch has no
...
// Only pull in the declarations when nothing has yet.
...
struct PosetailServerState {
    ...
    float last_encode_ms = 0.0f;      // crop + resize + PNG encode of all images
...
// One per-camera prepared image: PNG-encoded 256×256 RGB, with the K_scaled
...
// stb_image_write callback: append the emitted bytes to a std::vector.
inline void png_append_cb(void *ctx, void *data, int size) {
...
inline std::vector<uint8_t> encode_crop_png(
    ...
    for (int y = 0; y < S; ++y) {
        float fy = (y + 0.5f) * sy - 0.5f + (float)y0;
        int iy0 = std::clamp((int)std::floor(fy), 0, src_h - 1);
        int iy1 = std::min(src_h - 1, iy0 + 1);
        float wy = std::clamp(fy - iy0, 0.0f, 1.0f);
        for (int x = 0; x < S; ++x) {
            float fx = (x + 0.5f) * sx - 0.5f + (float)x0;
            int ix0 = std::clamp((int)std::floor(fx), 0, src_w - 1);
            int ix1 = std::min(src_w - 1, ix0 + 1);
            float wx = std::clamp(fx - ix0, 0.0f, 1.0f);
            const uint8_t *p00 = rgba + ((size_t)iy0 * src_w + ix0) * 4;
            const uint8_t *p01 = rgba + ((size_t)iy0 * src_w + ix1) * 4;
            const uint8_t *p10 = rgba + ((size_t)iy1 * src_w + ix0) * 4;
