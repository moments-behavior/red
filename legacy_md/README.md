...
| Feature | Details |
|---|---|
| GPU video decoding | NVIDIA hardware (h264/h265) via NVDEC; Apple VideoToolbox on macOS |
...
| Real-time 3D triangulation | Keypress triangulates labeled 2D points into world coordinates (DLT) |
...
```bash
brew install eigen ffmpeg glfw jpeg-turbo pkg-config
```

No CUDA, no OpenCV, no Vulkan, and no MoltenVK are required on macOS.
...
On Apple Silicon with a 3-camera, 180 fps, 3208x2200 dataset,
the application plays back at **1.0x real-time**. The pipeline is fully
GPU-side:
...
| LibTorch (CPU) | latest | YOLO model inference (optional) |
...
1. Use **Space** to play/pause. Use the arrow keys to step one frame at a time,
   or the seek slider at the top.
...
Create Project dialog and browse to your JSON file. See `example/skeleton.json`
